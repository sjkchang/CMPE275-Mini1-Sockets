...
void basic::BasicServer::start() {
  ...
  while (this->good) {
    ...
    if (incoming < 0) {
      ...
      err << "error on accept(), sock = " << this->svr << ", err = " << errno
          << std::endl;
    ...
    tv.tv_sec = 1;
    tv.tv_usec = 0;
    setsockopt(incoming, SOL_SOCKET,  SO_REUSEADDR | SO_REUSEPORT | SO_RCVTIMEO,
    (const char*)&tv, sizeof(tv));
    */

    int opt = 2;
    setsockopt(incoming, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &opt,
               sizeof(opt));
...
void basic::BasicServer::connect() {
  std::cerr << "configuring host: " << this->ipaddr << ", port: " << this->portN
            << std::endl;
  ...
  int opt = 1;
  auto stat =
      ::setsockopt(this->svr, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
  if (stat < 0) {
    std::stringstream err;
