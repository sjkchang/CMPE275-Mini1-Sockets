...
basic::BasicClient::BasicClient(std::string name, std::string ipaddr,
                                unsigned int port) {
  this->name = name;
  this->group = "public";
  this->ipaddr = ipaddr;
...
void basic::BasicClient::join(std::string group) { this->group = group; }

void basic::BasicClient::setName(std::string name) { this->name = name; }
...
void basic::BasicClient::sendMessage(std::string m) {
  ...
  while (this->good) {
    ...
    if (n == -1) {
      ...
      std::cerr << "sent: " << payload << ", size: " << plen
                << ", errno: " << errno << std::endl;
